...
export default class TagGroup extends React.Component {
  ...
  lookup(k, d) {
    return (
      (this.props.lookup && this.props.lookup[k] && this.props.lookup[k][d]) ||
      d
    )
  }
    ...
    _.map(tags, (f, dimension) => {
      _.map(f, (value, i) => {
        ...
        if (getColor) {
          iconEl = (
            <span
              className="graph-tags__element__color"
              style={{ backgroundColor: this.props.getColor(text, dimension) }}
            />
          )
        } else {
          iconEl = (
            <span className="graph-tags__element__icon">
        ...
        if (_.isObject(value)) {
          ...
          if (value.icon)
            iconEl = (
              <span className="graph-tags__element__icon">{value.icon}</span>
            )
        } else {
          text = value
        }
        ...
        filterEl.push(
          <span
            className={`graph-tags__element ${
              _.isUndefined(onChange) ? '' : 'graph-tags__element--with-action'
            }`}
