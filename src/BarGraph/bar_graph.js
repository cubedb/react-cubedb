...
class Bar extends React.Component {
  ...
  render() {
    ...
    if (this.props.group) {
        ...
        if (highlights.includes(bar.key)) {
          ...
          bars.push(
            <div
              ...
              title={`${bar.name}: ${numberFormat(bar.c)} (${(
                (bar.c / c) *
                100
              ).toFixed(3)}%)`}
      ...
      if (amount > 0) {
        const percentage =
          this.props.max > 0 ? (amount / this.props.max) * 100 : 0;
        const width = `${percentage}%`;
        bars.push(
          <div
            title={`others: ${numberFormat(amount)}, (${(
              (amount / c) *
              100
            ).toFixed(3)}%)`}
    ...
    return (
      <div className="bar-graph__bar-wrap">
        {bars}
        <span className="bar-graph__bar__percentage">{`${(
  ...
  render() {
    return (
      <div
        ...
        onClick={() =>
          this.props.onChange(this.props.name, this.props.data.key)
        }
      >
        {this.props.comparingData
          ? this.getIcon(this.props.data.c, this.props.comparingData.c)
          : null}
          ...
          <div className="bar-graph__bar-container">
            {this.props.comparingData ? (
              <Bar
                ...
                max={
                  this.props.stretched
                    ? this.props.comparingData.c
                    : this.props.max
                }
                getColor={this.props.getColor}
                proportion={
                  this.props.comparingTotal > 0 &&
                  this.props.comparingData.c > 0
                    ? (this.props.comparingData.c || 0) /
                      this.props.comparingTotal
                    : 0
                }
              />
            ) : null}
            <Bar
              ...
              getColor={this.props.getColor}
              proportion={
                this.props.total > 0 && this.props.data.c > 0
                  ? (this.props.data.c || 0) / this.props.total
                  : 0
              }
            />
          </div>
...
class BarList extends React.Component {
  ...
  render() {
    ...
    if (!_.isEmpty(serie)) {
      const comparingMax = this.props.comparingTo
        ? this.props.comparingTo.max
        : 0;
          ...
          {_(serie)
            ...
            .map(d => {
              ...
              if (this.props.comparingTo && this.props.comparingTo.serie) {
                comparingData = this.props.comparingTo.serie[d.key]
      ...
      return (
        <div className="cube_bars__list__content--empty">
...
class BarGraphHeader extends React.Component {
  ...
  onDownload = (dataLabel, volume, dataSerie, comparingTo) => () => {
    ...
    const createLine = (name, count = 0, stack = []) => {
      ...
      return `${name}${delimiter}${(proportion * 100).toFixed(
        3
      )}%${stacks}${endLine}`;
    };
    ...
    const body = _.map(dataSerie, el => {
      if (comparingTo) {
        ...
        return `${createLine(
          ...
          window.c,
          window.stack
        )}${createLine(`${el.name}${delimiter}B`, el.c, el.stack)}`;
      } else {
    ...
    saveData(
      `event_${this.props.name}_serie_${dataLabel}${
  ...
  onClickAddAll = e => {
    ...
    this.props.onChange(
      this.props.name,
      _.difference(this.props.dimensions, this.props.selectedItems)
    );
  };
  onClickInvert = e => {
    e.preventDefault();
  ...
  render() {
    const name = this.props.description ? (
      <OverlayTrigger
        placement="top"
        overlay={
          <Tooltip id={this.props.name}>{this.props.description}</Tooltip>
        }
      >
    ...
    return (
      <div>
        <h4>
          ...
          <small>
            (
            {(this.props.slice && this.props.slice < this.props.size
              ? `${this.props.slice} of `
              : "") + this.props.size}
            )
          </small>
        ...
        <ButtonGroup className="bar-graph__actions">
          ...
          <Button
            ...
            bsStyle={
              this.props.group === this.props.name ? "primary" : "default"
            }
            onClick={() => {
...
export default class BarGraph extends React.Component {
  ...
  render() {
    ...
    const isGroupSource =
      this.props.group && this.props.group === this.props.name;
    ...
    return (
      ...
      >
        ...
        <Panel.Body>
          {filter ? (
            <div className="bar-graph__search-help">
